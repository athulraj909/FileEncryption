...
from .encryption import generate_key, encrypt_file,decrypt_file,derive_key_from_password
import os
...
class DecryptionRequest(models.Model):
    ...
    def save(self, *args, **kwargs):
        # Ensure that decryption key is generated and saved before saving the instance
        ...
        if not self.id:  # New instance being created
            ...
            # Encrypt the file content using the generated key
            encrypted_content = decrypt_file(file_content, key)
            
            # Save the encrypted content back to the file
            with open(self.uploaded_file.path, 'wb') as file:
                file.write(encrypted_content)
